...
export const fetchInscriptionContent = async (id) => {
  try {
    ...
    const res = await fetchInscriptionContentFromBIS(id);
    if (res.ok) {
      const content = {
        id,
        simpleType:
          ALLOWED_MIME_TYPES_TO_SIMPLE_TYPES[res.headers.get('Content-Type')],
        type: res.headers.get('Content-Type'),
        content: Buffer.from(await res.arrayBuffer()).toString('hex'),
      };
      store.dispatch(addInscription(content));
      return content;
    } else {
      throw new Error('Error fetching inscription from CDN');
    }
  } catch (error) {
    try {
